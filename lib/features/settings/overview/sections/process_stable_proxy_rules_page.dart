...
class ProcessStableProxyRulesPage extends HookConsumerWidget {
  const ProcessStableProxyRulesPage({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final selected = ref.watch(ConfigOptions.effectiveProcessStableProxyRuleNames);
    ...
    Future<void> resetToCodexDefaults() async {
      await ref.read(ConfigOptions.processStableProxyRuleNames.notifier).reset();
    }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(tooltip: t.common.reset, onPressed: resetToCodexDefaults, icon: const Icon(Icons.restore_rounded)),
          IconButton(
  ...
  Future<void> _updateSelected(WidgetRef ref, List<String> processNames) async {
    await ref.read(ConfigOptions.processStableProxyRuleNames.notifier).update(_cleanSelectedProcessNames(processNames));
  }

  List<String> _cleanSelectedProcessNames(List<String> processNames) {
    return parseProcessStableProxyRuleNames(formatProcessStableProxyRuleNames(processNames));
  }
}
